...
def finalize(outdir):
    ...
    for cid,p in rep.groupby('confirm_id'):
        m,l,u=boot(p.value_of_information,make_generator(sett.root_seed,'p6-dboot',cid),sett.bootstrap);ds.append(dict(confirm_id=cid,design_id=p.design_id.iloc[0],voi_mean=m,voi_low=l,voi_high=u,identity_exact=bool(p.identity_information_recovered.all() and p.identity_viability_recovered.all()),target_fraction=float(p.target_reached.mean()),positive=bool(l>DV)))
